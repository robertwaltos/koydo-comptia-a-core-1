...
import { EXAM_CONFIG } from "@/lib/comptia-a-core-1/config";

describe("CompTIA A+ Core 1 Config", () => {
  it("has correct exam ID", () => {
    expect(EXAM_CONFIG.examId).toBe("EXAM151");
  });

  it("has correct slug", () => {
    expect(EXAM_CONFIG.slug).toBe("comptia-a-core-1");
  });
